...
class CompositeTransform(beam.PTransform):
    def expand(self, pcollection):
        filtered = pcollection | beam.Filter(
            lambda row: row.transaction_amount > 20 and row.timestamp.year >= 2010
        )
        return filtered | beam.GroupBy(
            date=lambda row: str(row.timestamp.date())
